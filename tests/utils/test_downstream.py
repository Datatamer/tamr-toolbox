...
@mock_api(enforce_online_test=ONLINE_TEST)
def test_get_dataset_downstream_dependencies():
    ...
    assert PIPELINE_DEPENDENT_DATASETS.issubset(set([d.name for d in downstream_datasets]))
...
def test_get_dataset_downstream_dependencies_suggest_name():
    ...
    assert DEPENDENT_DATASETS_BY_NAME.issubset(set([d.name for d in downstream_datasets]))
...
def test_get_downstream_projects():
    ...
    assert (
        str(downstream_projects) == "[tamr_unify_client.project.resource.Project(relative_id="
...
def test_get_downstream_datasets_for_source_dataset():
    ...
    unified_dataset = client.datasets.by_resource_id(MASTERING_UNIFIED_DATASET_ID)
    source_downstream_datasets = tamr_toolbox.utils.downstream.datasets(source_dataset)
    unified_downstream_datasets = tamr_toolbox.utils.downstream.datasets(unified_dataset)
    source_downstream_datasets_names = set([d.name for d in source_downstream_datasets])
    unified_downstream_datasets_names = set([d.name for d in unified_downstream_datasets])
    # All downstream datasets of unified dataset must be downstream datasets of the source dataset
    assert unified_downstream_datasets_names.issubset(source_downstream_datasets_names)
...
def test_get_downstream_datasets_for_source_dataset_suggest_name():
    ...
    unified_downstream_datasets = tamr_toolbox.utils.downstream.datasets(
        unified_dataset, include_dependencies_by_name=True
    )
    source_downstream_datasets_names = set([d.name for d in source_downstream_datasets])
    unified_downstream_datasets_names = set([d.name for d in unified_downstream_datasets])
    # All downstream datasets of unified dataset must be downstream datasets of the source dataset
