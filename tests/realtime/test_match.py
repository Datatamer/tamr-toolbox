...
def test_match_query_with_no_match(type: str):
    match_client = utils.client.create(**CONFIG["toolbox_realtime_match_instance"])
    result = match_query(
        match_client, project_name="minimal_mastering", records=[{"ssn": "0000"}], type=type,
    )
    assert result[0] == []
    return None
...
def test_match_no_input_data():
    ...
    with pytest.warns(
        ...
        result = match_query(
            match_client, project_name="minimal_mastering", records=[], type="records",
        )
    assert len(result) == 0
    return None
...
def test_poll_match_status():
    ...
    queryable = poll_realtime_match_status(match_client, project_name="minimal_mastering")
    assert queryable
    # Test project that is not queryable
    queryable = poll_realtime_match_status(
        match_client, project_name="minimal_incomplete_mastering"
    )
    assert ~queryable
    return None
...
def test_match_min_match_prob(type: str):
    ...
    full_result = match_query(
        match_client, project_name="minimal_mastering", records=MATCH_TEST_DATA, type=type,
    )
