...
def main(
    *, instance_connection_info: Dict[str, str], pipeline_vars: Dict[str, Any]
    ...
    preprocessing_projects = [
        tamr_client.projects.by_resource_id(p_id) for p_id in preprocessing_project_ids
    ]
    ...
    pin_DeltasDataset = tamr_client.projects.client.post(
        ...
        + pipeline_vars["GR_mapping_delta_dataset"]
        + "/advancePinnedVersion"
    )
    ...
    upd_GR_MapDeltas_sts = tbox.project.schema_mapping.jobs.update_unified_dataset(
        tamr_client.projects.by_resource_id(
            project_id_dict[pipeline_vars["GR_mapping_delta_project"]]
        )
    )
    ...
    upd_GR_Out_sts = tbox.project.schema_mapping.jobs.update_unified_dataset(
        tamr_client.projects.by_resource_id(
            project_id_dict[pipeline_vars["GR_output_delta_project"]]
        )
    )
    if upd_GR_Out_sts[0].state == "SUCCEEDED":
